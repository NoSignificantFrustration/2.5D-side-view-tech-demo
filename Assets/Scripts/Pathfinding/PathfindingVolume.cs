    ...
    public void CreateGrid()
    {
        ...
        for (int y = 0; y < gridSize.y; y++)
        {
            for (int x = 0; x < gridSize.x; x++)
            {
                ...
                gridTraversableArray.Set(arrayPos, !Physics.CheckSphere(worldPoint, cellRadius - 0.01f, groundMask));
    ...
    public void RefreshGridWalkableArray(Vector2Int bottomLeftStart, Vector2Int topRightEnd)
    {
        ...
        for (int y = bottomLeftStart.y; y < topRightEnd.y; y++)
        {
            for (int x = bottomLeftStart.x; x < topRightEnd.x; x++)
            {
                ...
                gridTraversableArray.Set(arrayPos, !Physics.CheckSphere(worldPoint, cellDiameter/2 - 0.01f, groundMask));
            }
        }
